...
import WAMWebView from './WAMWebViewNativeComponent.android';
import {
  defaultOriginWhitelist,
  defaultRenderError,
  defaultRenderLoading,
  useWebWiewLogic,
} from './WAMWebViewShared';
import { AndroidWebViewProps, NativeWebViewAndroid } from './WAMWebViewTypes';

import styles from './WAMWebView.styles';
  ...
  (
    ...
    const onShouldStartLoadWithRequestCallback = useCallback(
      (shouldStart: boolean, url: string, lockIdentifier?: number) => {
        if (lockIdentifier) {
          NativeModules.WAMWebView.onShouldStartLoadWithRequestCallback(
            shouldStart,
            lockIdentifier,
          );
        } else if (shouldStart) {
    ...
    if (viewState === 'LOADING') {
      ...
      console.error(`WAMWebView invalid state encountered: ${viewState}`);
    }
    ...
    const NativeWebView =
      (nativeConfig?.component as typeof NativeWebViewAndroid | undefined) ||
      WAMWebView;
...
const isFileUploadSupported: () => Promise<boolean> =
  NativeModules.WAMWebView.isFileUploadSupported();
